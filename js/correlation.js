...
import { filterByGender, correlation, getDepressionSummary, getWorkGroup, cleanSleepLabel, getDepressionRate, sortSleep } from './exports/utils.js';
...
for (let row of data) {
  let h = Number(row.workStudyHours);
  let s = sleepMap[row.sleepDuration];
  let d = row.depression === 'Yes' ? 1 : 0;

  if (!isNaN(h) && s !== undefined) {
    hoursArr.push(h);
    sleepArr.push(s);
    depressionArr.push(d);
    depArr.push(d);
  }
}
